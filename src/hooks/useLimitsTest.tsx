...
interface LimitsHook {
  ...
  isLoading: boolean;
  // Funciones de prueba
  setTestUsage: (usage: Partial<Usage>) => void;
  resetTestUsage: () => void;
}

export const useLimitsTest = (): LimitsHook => {
  const { user } = useAuth();
  ...
  // Funciones de prueba
  const setTestUsage = (usage: Partial<Usage>) => {
    setCurrentUsage(prev => ({ ...prev, ...usage }));
  };

  const resetTestUsage = () => {
